...
study_zip_files_data_file_uuid_tsv = path.join( study_out_dir, 'Study.data_file_uuid.from_studyZipFileQuery.zip_files.tsv' )
...
data_file_study_zip_files_tsv = path.join( data_file_out_dir, 'DataFile.from_studyZipFileQuery.zip_files.tsv' )

json_out_dir = path.join( output_root, '__API_result_json' )
studyZipFileQuery_json_output_file = path.join( json_out_dir, 'studyZipFileQuery.json' )
...
studyZipFileQuery_query = '''
query search {
    studyZipFileQuery ( study_short_name: "$STUDY_SHORT_NAME$" ) {
        study_short_name
        study_id
        study_accession
        data_file_type
...
output_tsv_keywords = [
    'DATA_FILE_FROM_STUDY_ZIP_FILES',
    'STUDY_STUDY_ZIP_FILE'
]

output_tsv_filenames = [
    data_file_study_zip_files_tsv,
    study_zip_files_data_file_uuid_tsv
]

output_tsvs = dict( zip( output_tsv_keywords, [ open( file_name, 'w' ) for file_name in output_tsv_filenames ] ) )

# Table headers.
print( *scalar_data_file_fields, sep='\t', end='\n', file=output_tsvs['DATA_FILE_FROM_STUDY_ZIP_FILES'] )
print( *[ 'study_id', 'data_file_uuid' ], sep='\t', end='\n', file=output_tsvs['STUDY_STUDY_ZIP_FILE'] )

with open( studyZipFileQuery_json_output_file, 'w' ) as JSON:
    
    for study_short_name in sorted( study_short_name_to_study_id ):
        # Send the studyZipFileQuery() query to the API server.
        response = requests.post(
            ctdc_api_url,
            json={
                'query': re.sub( '\\$STUDY_SHORT_NAME\\$', study_short_name, studyZipFileQuery_query ),
                'variables': {}
            },
            headers={
        ...
        if not response.ok:
            print( re.sub( '\\$STUDY_SHORT_NAME\\$', study_short_name, studyZipFileQuery_query ), file=sys.stderr )
        ...
        for study_zip_file in result['data']['studyZipFileQuery']:
            study_id = study_short_name_to_study_id[study_zip_file['study_short_name']]

            for data_file in study_zip_file['zip_files']:
                # This can happen.
