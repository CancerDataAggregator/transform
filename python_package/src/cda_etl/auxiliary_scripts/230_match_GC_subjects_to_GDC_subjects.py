...
project_map_tsv = path.join( agg_project_dir, 'naive_GC_GDC_project_id_map.hand_edited_to_remove_false_positives.tsv' )

agg_subject_dir = path.join( agg_root, 'subjects' )

output_file = path.join( agg_subject_dir, 'GC_CDA_subjects_linked_to_GDC_CDA_subjects.tsv' )
...
# cda_table	id_alias	upstream_source	upstream_field	upstream_id
# subject	48470	GC	participant.uuid	dd1a5308-2a5f-5c73-9984-c374b5111c5e
...
# Load hand-verified links between GDC projects and GC studies.

gc_study_id_to_gdc_project_id = map_columns_one_to_many( project_map_tsv, 'GC_study_uuid', 'GDC_project_id' )

# Save ID information for all records merged due to ( same case_submitter_id ) + ( allowed equivalence between containing GDC project and containing GC study ).

gc_subject_alias_to_gdc_subject_alias = dict()
...
with open( gc_entity_project_map ) as IN:
    ...
    for record in [ dict( zip( column_names, line.rstrip( '\n' ).split( '\t' ) ) ) for line in IN ]:
        ...
        if record['entity_type'] == 'participant':
            ...
            if study_id in gc_study_id_to_gdc_project_id:
                
                for gdc_project_id in gc_study_id_to_gdc_project_id[study_id]:
                    ...
                    if gdc_project_id in gdc_project_id_and_case_submitter_id_to_cda_subject_alias and participant_submitter_id in gdc_project_id_and_case_submitter_id_to_cda_subject_alias[gdc_project_id]:
                        ...
                        if cda_subject_alias in gc_subject_alias_to_gdc_subject_alias and gc_subject_alias_to_gdc_subject_alias[cda_subject_alias] != gdc_subject_alias:
                            
                            sys.exit( f"FATAL: One GC CDA subject_alias ({cda_subject_alias}) mapped to two GDC CDA subject aliases ({gc_subject_alias_to_gdc_subject_alias[cda_subject_alias]} and {gdc_subject_alias}); aborting." )

                        else:
                            
                            gc_subject_alias_to_gdc_subject_alias[cda_subject_alias] = gdc_subject_alias

# Write results.

with open( output_file, 'w' ) as OUT:
    
    print( *[ 'GC_subject_alias', 'GC_subject_id', 'GDC_subject_alias', 'GDC_subject_id' ], sep='\t', file=OUT )

    for gc_subject_alias in sorted( gc_subject_alias_to_gdc_subject_alias ):
        
        gc_subject_id = gc_subject_alias_to_subject_id[gc_subject_alias]

        gdc_subject_alias = gc_subject_alias_to_gdc_subject_alias[gc_subject_alias]
