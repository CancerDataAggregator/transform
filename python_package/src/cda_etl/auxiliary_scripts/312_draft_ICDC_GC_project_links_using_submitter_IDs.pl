...
die( "\n   Usage: $0 <ICDC file> <GC file> <output file>\n\n" ) if ( not -e $icdc_file or not -e $gc_file or $out_file eq '' );
...
foreach my $target_type ( keys %$icdc_data ) {
    ...
    foreach my $entity_id ( keys %{$icdc_data->{$target_type}} ) {
        ...
        foreach my $icdc_study_name ( keys %{$icdc_data->{$target_type}->{$entity_id}} ) {
            ...
            if ( exists( $gc_data->{$target_type} ) ) {
                
                if ( exists( $gc_data->{$target_type}->{$entity_id} ) ) {
                    
                    foreach my $gc_study_name ( keys %{$gc_data->{$target_type}->{$entity_id}} ) {
                        
                        if ( exists( $icdc_to_gc_count->{$icdc_study_name}->{$gc_study_name} ) ) {
                            
                            $icdc_study_total_match_count->{$icdc_study_name} += 1;

                            $icdc_to_gc_count->{$icdc_study_name}->{$gc_study_name} += 1;
...
print OUT join( "\t", 'match_count', 'ICDC_program_acronym', 'ICDC_program_name', 'ICDC_study_clinical_study_designation', 'ICDC_study_name', 'GC_program_acronym', 'GC_study_name', 'GC_study_phs_accession', 'GC_study_uuid' ) . "\n";
...
foreach my $icdc_program_acronym ( sort { $icdc_program_total_match_count->{$b} <=> $icdc_program_total_match_count->{$a} } keys %$icdc_program_total_match_count ) {
    ...
    foreach my $icdc_study_name ( sort { $icdc_study_total_match_count->{$b} <=> $icdc_study_total_match_count->{$a} } keys %{$icdc_program_to_study->{$icdc_program_acronym}} ) {
        ...
        foreach my $gc_study_name ( sort { $a cmp $b } keys %{$icdc_to_gc_count->{$icdc_study_name}} ) {
            
            my $match_count = $icdc_to_gc_count->{$icdc_study_name}->{$gc_study_name};

            my $gc_program_acronym = $gc_study_to_program->{$gc_study_name};

            my $gc_study_phs_accession = $gc_study_name_to_phs_accession->{$gc_study_name};

            my $gc_study_uuid = $gc_study_name_to_uuid->{$gc_study_name};

            print OUT join( "\t", $match_count, $icdc_program_acronym, $icdc_program_name, $icdc_study_id, $icdc_study_name, $gc_program_acronym, $gc_study_name, $gc_study_phs_accession, $gc_study_uuid ) . "\n";
        }
    }
}
