...
then
    ...
    echo
    echo "   ERROR: Usage: $script_name <CDA version name, e.g. \"September 2026\">"
    echo
    exit 0
fi
...
ctdc_extraction_date_file=./extracted_data/ctdc_postprocessed/extraction_date.txt
ctdc_extraction_date_string=`cat $ctdc_extraction_date_file`
ctdc_version_file=./extracted_data/ctdc_postprocessed/data_version_as_fetch_date.txt
ctdc_version_string=`cat $ctdc_version_file`

input_root=./cda_tsvs/last_merge
summary_output_dir=./auxiliary_metadata/__column_value_statistics/merged_CDA_data/GDC_${gdc_extraction_date_string}_plus_PDC_${pdc_extraction_date_string}_plus_GC_${gc_extraction_date_string}_plus_ICDC_${icdc_extraction_date_string}_plus_IDC_${idc_extraction_date_string}_plus_CTDC_${ctdc_extraction_date_string}/dataset_summaries
summary_file="${summary_output_dir}/GDC_plus_PDC_plus_GC_plus_ICDC_plus_IDC_plus_CTDC.converted_to_CDA.harmonized.merged.counts_by_upstream_data_source.tsv"
current_date=$(date '+%Y-%m-%d')

echo ./package_root/auxiliary_scripts/901_summarize_distinct_values_by_table_and_column.source_aware.py $input_root $summary_file "${cda_version_string}" $current_date print_CDA_rows gdc $gdc_extraction_date_string "${gdc_version_string}" pdc $pdc_extraction_date_string "${pdc_version_string}" gc $gc_extraction_date_string "${gc_version_string}" icdc $icdc_extraction_date_string "${icdc_version_string}" idc $idc_extraction_date_string "${idc_version_string}" ctdc $ctdc_extraction_date_string "${ctdc_version_string}"
