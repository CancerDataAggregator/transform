...
job_config = bigquery.QueryJobConfig( use_query_cache=True, allow_large_results=True )
# google.cloud.bigquery.client.Client
bq_client = bigquery.Client( project_id )

with gzip.open( output_file, 'wt' ) as OUT:
    while not no_more_rows:
        #selection_query = f"SELECT DISTINCT {field_list_string} FROM {target_table_path} ORDER BY {order_by_field_sequence} LIMIT {limit} OFFSET {offset}"
        selection_query = f"SELECT DISTINCT {field_list_string} FROM {target_table_path}"
        print( f"Querying\n\n    {selection_query}\n\ninto\n\n    {output_file} ...", end='\n\n', file=sys.stderr )
        ...
        if result_rows.total_rows == 0:
            ...
            for row in result_rows:
                ...
                OUT.write( '\n' )
                # Remove this if you go back to paging
                no_more_rows = True
        #offset = offset + limit
