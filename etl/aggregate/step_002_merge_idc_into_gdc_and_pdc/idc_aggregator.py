...
from etl.lib import map_columns_one_to_one, sort_file_with_header
...
class IDC_aggregator:
    ...
    def __init__( self, source_version ):
        ...
        self.aux_dir = path.join( self.extract_dir, '__supplemental_metadata' )
        self.log_dir = path.join( self.extract_dir, '__processing_logs' )

        self.input_files = {
            
            'gdc_project_affiliations' : path.join( self.aux_dir, 'GDC_entity_submitter_id_to_program_name_and_project_id.tsv' ),
            'gdc_to_idc_project_map' : path.join( self.aux_dir, 'naive_GDC-IDC_project_id_map.hand_edited.tsv' ),
            'pdc_project_affiliations' : path.join( self.aux_dir, 'PDC_entity_submitter_id_to_program_project_and_study.tsv' ),
            'idc_to_pdc_project_map' : path.join( self.aux_dir, 'naive_IDC-PDC_project_id_map.hand_edited.tsv' ),
            'gdc_pdc_merged_subject_ids' : path.join( self.aux_dir, 'PDC_subject_IDs_absorbed_into_corresponding_GDC_subject_IDs.tsv' ),
        ...
        self.aux_files = {
            ...
            'idc_subject_id_to_gdc_subject_id' : path.join( self.aux_dir, 'idc_subject_id_to_gdc_subject_id.tsv' ),
            'idc_subject_id_to_pdc_subject_id' : path.join( self.aux_dir, 'idc_subject_id_to_pdc_subject_id.tsv' ),
            'all_merged_subject_ids' : path.join( self.aux_dir, 'all_merged_subject_ids.tsv' ),
            'idc_subject_id_to_cda_subject_id' : path.join( self.aux_dir, 'idc_subject_id_to_cda_subject_id.tsv' )
        }

        for target_dir in [ self.merged_output_dir, self.aux_dir, self.log_dir ]:
    ...
    def merge_into_existing_cda_data( self ):
        ...
        log_dir = path.join( self.merged_output_dir, '__merge_logs' )

        subject_merged_log = path.join( log_dir, 'IDC_subject_IDs_absorbed_into_corresponding_GDC_PDC_merged_subject_IDs.tsv' )

        null_sub_log = path.join( log_dir, 'IDC_values_replacing_GDC_PDC_merged_nulls_by_subject_column_name.tsv' )

        clashes_ignored_log = path.join( log_dir, 'IDC_values_unused_but_different_from_GDC_PDC_merged_values_by_subject_column_name_and_dataset.tsv' )

        for target_dir in [ log_dir ]:
