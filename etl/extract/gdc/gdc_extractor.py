...
from etl.lib import add_to_map, associate_id_list_with_parent, get_safe_value, singularize, sort_file_with_header, update_field_lists, write_association_pairs
...
class GDC_extractor:
    ...
    def __init__( self, endpoint, source='gdc', refresh=True ):
        ...
        endpoint_config = __import__( f"etl.extract.gdc.config.{endpoint}", fromlist=[None] )
    ...
    def __get_endpoint_JSON( self, parameters ):
        ...
        if result.ok:
            ...
            sys.exit( f"FATAL: API call failed. Aborting.")
