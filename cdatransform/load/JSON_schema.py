...
class Schema:
    ...
    def _init_data_model(self, data_model_dir):
        print('ran init data model')
        data_model = dict({}) 
        for entity in list(self.mapping.keys()):
            f = open(data_model_dir + '/' + entity + '.json',)
            data = json.load(f)
    ...
    def add_identifier(self, entity):
        ...
        for field in list(self.mapping[entity]['Mapping']['identifier'].keys()):
            field_dict = dict({})
            field_dict['description'] =  self.data_model['identifier'][field]['description']
    ...
    def make_entity_schema(self, entity):
        ...
        for field in list(self.mapping[entity]['Mapping'].keys()):
            field_dict = dict({})
            field_dict['description'] =  self.data_model[entity].get(field,dict({})).get('description','')
            field_dict['name'] = field
            if field == 'id':
            ...
            if self.data_model[entity].get(field,dict({})).get('oneOf') is not None:
                field_dict['type'] = self.data_model[entity].get(field,dict({})).get('oneOf',[dict({'type': 'string'})])[1]['type'].upper()
                field_dict['mode'] = 'REPEATED'
            else:
                field_dict['type'] = self.data_model[entity].get(field, dict({})).get('type', '').upper()
                if field_dict['type'] == '' and self.data_model[entity].get(field,dict({})).get('$ref', None) is not None:
                    field_dict['type'] = 'STRING'
            if field in ['associated_project', 'species', 'drs_uri', 'data_modality', 'imaging_modality','primary_disease_type']:
                field_dict['type'] = 'STRING'
...
def main():
    parser = argparse.ArgumentParser(description="Generate JSON schema using mapping and cda-data-model json files.")
    ...
    parser.add_argument("data_model_dir", help="location of directory containing cda-data-model json schemas")
