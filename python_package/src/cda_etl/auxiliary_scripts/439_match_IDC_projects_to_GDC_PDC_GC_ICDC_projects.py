...
idc_gc_merge_map = path.join( aux_dir, 'IDC_CDA_projects_linked_to_GC_CDA_projects.tsv' )
...
output_file = path.join( aux_dir, 'IDC_CDA_projects_merged_into_GDC_PDC_GC_ICDC_CDA_projects.tsv' )
...
merged_id = map_columns_one_to_one( gdc_pdc_gc_icdc_merge_map, 'new_project_alias', 'new_project_id' )

for merge_file in [ gdc_pdc_gc_merge_map, gdc_pdc_merge_map ]:
...
idc_to_gc = map_columns_one_to_one( idc_gc_merge_map, 'IDC_project_alias', 'GC_project_alias' )
...
with open( output_file, 'w' ) as OUT:
    ...
    print( *[ 'IDC_project_alias', 'IDC_project_id', 'GDC_PDC_GC_ICDC_project_alias', 'GDC_PDC_GC_ICDC_project_id', 'new_project_alias', 'new_project_id' ], sep='\t', file=OUT )
    ...
    for idc_project_alias in sorted( idc_project_id ):
        ...
        if idc_project_alias in idc_to_icdc:
            ...
            if idc_project_alias in idc_to_gc:
                
                gc_target_to_match = idc_to_gc[idc_project_alias]
                ...
                if gc_target_to_match != icdc_target_to_match:
                    ...
                    sys.exit( f"FATAL: IDC project alias {idc_project_alias} matched (via GC) to {gc_target_to_match} but also (via ICDC) to {icdc_target_to_match}; cannot continue, aborting." )
        ...
        elif idc_project_alias in idc_to_gc:
            ...
            if idc_project_alias in idc_to_pdc:
                ...
                if pdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: IDC project alias {idc_project_alias} matched (via PDC) to {pdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
            ...
            if idc_project_alias in idc_to_gdc:
                ...
                if gdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: IDC project alias {idc_project_alias} matched (via GDC) to {gdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
