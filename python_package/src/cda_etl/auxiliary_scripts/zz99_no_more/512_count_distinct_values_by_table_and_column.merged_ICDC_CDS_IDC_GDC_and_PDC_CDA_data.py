...
input_root = path.join( 'cda_tsvs', 'merged_icdc_cds_idc_gdc_and_pdc_tables' )

aux_root = 'auxiliary_metadata'

stats_dir = path.join( aux_root, '__column_stats' )

stats_file = path.join( stats_dir, 'ICDC_CDS_IDC_GDC_and_PDC_column_stats_by_table.converted_to_CDA_and_merged.tsv' )
...
with open( stats_file, 'w' ) as STATS_FILE:
    ...
    print( 'Tabulating stats for (nearly) all CDA fields from ICDC+CDS+IDC+GDC+PDC...', end='\n\n', file=sys.stderr )
    ...
    for file_basename in sorted( listdir( input_root ) ):
        ...
        if re.search( r'\.tsv$', file_label ) is not None and ( re.search( r'_', file_label ) is None or re.search( '_associated_project\.tsv$', file_label ) is not None ):
            ...
            print( f"   ...{file_label}...", file=sys.stderr )
    ...
    print( '\n...done computing general stats on CDA field values from ICDC+CDS+IDC+GDC+PDC.', end='\n\n', file=sys.stderr )
