...
study_list_type_tsv = path.join( study_out_dir, 'Study.list_type.from_participantAndBiospecimenFilesByStudyId.tsv' )
study_data_file_uuid_tsv = path.join( study_out_dir, 'Study.data_file_uuid.from_participantAndBiospecimenFilesByStudyId.data_files.tsv' )

study_dict = load_tsv_as_dict( study_tsv )

data_file_out_dir = path.join( output_root, 'DataFile' )
data_file_tsv = path.join( data_file_out_dir, 'DataFile.from_participantAndBiospecimenFilesByStudyId.data_files.tsv' )
data_file_association_tsv = path.join( data_file_out_dir, 'DataFile.association.from_participantAndBiospecimenFilesByStudyId.data_files.tsv' )

json_out_dir = path.join( output_root, '__API_result_json' )
participantAndBiospecimenFilesByStudyId_json_output_file = path.join( json_out_dir, 'participantAndBiospecimenFilesByStudyId.json' )
...
# Non-scalar AssocDataFile fields:
#     association: [String]
# At time of writing (2026-09-14), this is the only endpoint query that correctly retrieves multiple values from DataFile.association.
# Presumably this is not unrelated to the recent creation of the 'AssocDataFile' entity (whose data we are saving as DataFile records to save sanity).
...
participantAndBiospecimenFilesByStudyId_query = '''
query search {
...
with open( participantAndBiospecimenFilesByStudyId_json_output_file, 'w' ) as JSON:
    
    for study_id in sorted( study_dict.keys() ):
        # Send the participantAndBiospecimenFilesByStudyId() query to the API server.
        response = requests.post(
            ctdc_api_url,
            json={
                'query': re.sub( '\\$STUDY_ID\\$', study_id, participantAndBiospecimenFilesByStudyId_query ),
                'variables': {}
            },
            headers={
        ...
        if not response.ok:
            print( re.sub( '\\$STUDY_ID\\$', study_id, participantAndBiospecimenFilesByStudyId_query ), file=sys.stderr )
        ...
        for participant_and_biospecimen_files_by_study_id in result['data']['participantAndBiospecimenFilesByStudyId']:
            # ParticipantAndBiospecimenFilesByStudyId.list_type [array of strings]. Associate with the Study record referenced in this ParticipantAndBiospecimenFilesByStudyId object.
            if 'list_type' in participant_and_biospecimen_files_by_study_id and len( participant_and_biospecimen_files_by_study_id['list_type'] ) > 0:
                for list_type in participant_and_biospecimen_files_by_study_id['list_type']:
                    print( *[ study_id, list_type ], sep='\t', end='\n', file=output_tsvs['STUDY_LIST_TYPE'] )
            for data_file in participant_and_biospecimen_files_by_study_id['data_files']:
                # This can happen.
                if 'data_file_uuid' in data_file and data_file['data_file_uuid'] is not None and data_file['data_file_uuid'] != '':
                    ...
                    if 'association' in data_file and data_file['association'] is not None and len( data_file['association'] ) > 0:
                        for association in data_file['association']:
                            print( *[ data_file['data_file_uuid'], association ], sep='\t', end='\n', file=output_tsvs['DATA_FILE_ASSOCIATION'] )
