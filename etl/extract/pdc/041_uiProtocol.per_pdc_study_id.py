...
protocol_tsv = f"{protocol_out_dir}/Protocol.from_uiProtocol.per_pdc_study_id.tsv"
protocol_study_tsv = f"{protocol_out_dir}/Protocol.study.from_uiProtocol.per_pdc_study_id.tsv"
protocol_pdc_study_id_tsv = f"{protocol_out_dir}/Protocol.pdc_study_id.from_uiProtocol.per_pdc_study_id.tsv"

uiProtocol_json_output_file = f"{json_out_dir}/uiProtocol.per_pdc_study_id.json"
...
with open(uiProtocol_json_output_file, 'w') as JSON:
    ...
    print( *('protocol_id', 'pdc_study_id'), sep='\t', end='\n', file=output_tsvs['PROTOCOL_PDC_STUDY_ID'] )

    for pdc_study_id in pdc_study_ids:
        
        api_query_json = {
            'query': '''            {
                uiProtocol( ''' + f'pdc_study_id: "{pdc_study_id}"' + ''' ) {
        ...
        # Send the uiProtocol() query to the API server.
        ...
        for protocol in result['data']['uiProtocol']:
            ...
            print( *(protocol['protocol_id'], pdc_study_id), sep='\t', end='\n', file=output_tsvs['PROTOCOL_PDC_STUDY_ID'] )
