        ...
        for subject in reader:
            ...
            if 'ResearchSubject' in subject:
                for researchsubject in subject['ResearchSubject']:
                    researchsubject_entity = initialize_entity(researchsubject,mapping,'ResearchSubject')
                    ...
                    if 'Specimen' in researchsubject:  
                        for specimen in researchsubject['Specimen']:
                            specimen_entity = initialize_entity(specimen,mapping,'Specimen')
                            specimen_entity['Subjects'] = [subject['id']]
                            specimen_entity['ResearchSubjects'] = [researchsubject['id']]
                    ...
                    if 'Diagnosis' in researchsubject:  
                        for diagnosis in researchsubject['Diagnosis']:
                            diagnosis_entity = initialize_entity(diagnosis,mapping,'Diagnosis')
                            diagnosis_entity['Subjects'] = [subject['id']]
                            diagnosis_entity['ResearchSubjects'] = [researchsubject['id']]
                            diagnosis_entity['Treatments'] = []
                            if 'Treatment' in diagnosis:
                                for treatment in diagnosis['Treatment']:
                                    treatment_entity = initialize_entity(treatment,mapping,'Treatment')
                                    treatment_entity['Subjects'] = [subject['id']]
                                    treatment_entity['ResearchSubjects'] = [researchsubject['id']]
                                    treatment_entity['Diagnoses'] = [diagnosis['id']]
                                    treatments[treatment_entity['id']].append(treatment_entity)
                                    diagnosis_entity['Treatments'].append(treatment_entity['id'])
                                    researchsubject_entity['Treatments'].append(treatment_entity['id'])
                                    subject_entity['Treatments'].append(treatment_entity['id'])
                    ...
                    researchsubjects[researchsubject_entity['id']].append(researchsubject_entity)
                    subject_entity['ResearchSubjects'].append(researchsubject_entity['id'])
...
def main():
    ...
    parser.add_argument("merged_entities_file", help="Name of merged entities file.Should end with .gz")
    parser.add_argument("endpoint", help="Subjects or Files")
    parser.add_argument("merge_file", help="Mapping file name. Contains entity information")
