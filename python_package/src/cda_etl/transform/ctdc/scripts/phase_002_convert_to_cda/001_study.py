...
extracted_tsv_input_root = path.join( 'extracted_data', f"{upstream_data_source.lower()}_processed" )
...
collated_tsv_input_root = path.join( 'extracted_data', f"{upstream_data_source.lower()}_postprocessed" )
...
for study_id in study:
    study_short_name = study[study_id]['study_short_name']
    study_name = study[study_id]['study_name']
    study_accession = study[study_id]['study_accession']
    ...
    cda_project_record['short_name'] = study_short_name
    ...
    upstream_identifiers[cda_id]['Study.study_short_name'] = study_short_name
    # This is never null at time of writing (2026-09-21); see GC code for conditional processing if that changes. For now, a warning will be thrown below if not present.
    upstream_identifiers[cda_id]['Study.study_accession'] = study_accession

    # Junk is possible in this field; scan for recognizability.
    if re.search( r'^.*phs[0-9]+', study_accession ) is None:
        print( f"WARNING: 'Study.study_accession' field for study_id '{study_id}' is '{study_accession}', which is not a valid phs_accession. Skipping CDA project table update (but logging bad value in upstream_identifiers)." )
    else:
        # Create separate CDA project records for all dbGaP studies listed as cross-references.
        dbgap_study_accession = re.sub( r'^.*(phs[0-9]+)([^0-9].*)?$', r'\1', study_accession )
        hit_dbgap = False
        cda_dbgap_id = f"dbGaP.study_accession.{dbgap_study_accession}"

        # Model CTDC Studies as subprojects of the dbGaP studies which which they have been directly associated:
