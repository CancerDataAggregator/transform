...
input_dir = path.join( 'extracted_data', 'gc_initial_database_dump' )
extraction_date_file = path.join( input_dir, 'gc_extraction_date.txt' )
version_file = path.join( input_dir, 'gc_version.txt' )

extraction_dir = path.join( 'extracted_data', 'gc' )
metadata_dir = path.join( extraction_dir, '__GC_release_metadata' )

# EXECUTION

if not path.isdir( input_dir ):
    sys.exit( f"FATAL: Required GC database-dump directory '{input_dir}' not found; aborting." )
...
for file_basename in sorted( listdir( input_dir ) ):
    if re.search( r'\.cypherl$', file_basename, flags=re.IGNORECASE ) is not None:
...
if not found_version_file:
    sys.exit( f"FATAL: Cannot locate required GC version-name file '{version_file}'; aborting." )
