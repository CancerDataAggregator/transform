...
out_json = path.join( output_dir, 'CTDC_schema.json' )
out_gql = path.join( output_dir, 'CTDC_schema.gql' )
...
if response.ok:
    schema = json.loads( response.content )['data']['__schema']
    ...
    with open( out_gql, 'w' ) as OUT:
        ...
        for currentType in schema['types']:
    ...
    # If response code is not ok (200), echo the query and print the resulting http error code with description
    print( query )
