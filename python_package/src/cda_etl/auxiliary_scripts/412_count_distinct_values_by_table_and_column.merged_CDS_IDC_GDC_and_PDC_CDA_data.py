...
input_root = path.join( 'cda_tsvs', 'merged_cds_idc_gdc_and_pdc_tables' )

aux_root = 'auxiliary_metadata'

stats_dir = path.join( aux_root, '__column_stats' )

stats_file = path.join( stats_dir, 'CDS_IDC_GDC_and_PDC_column_stats_by_table.converted_to_CDA_and_merged.tsv' )
...
with open( stats_file, 'w' ) as STATS_FILE:
    ...
    print( 'Tabulating stats for (nearly) all CDA fields from CDS+IDC+GDC+PDC...', end='\n\n', file=sys.stderr )
    ...
    print( '\n...done computing general stats on CDA field values from CDS+IDC+GDC+PDC.', end='\n\n', file=sys.stderr )
