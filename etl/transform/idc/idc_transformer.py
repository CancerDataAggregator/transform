...
from etl.lib import load_tsv_as_dict, map_columns_one_to_one, sort_file_with_header
...
class IDC_transformer:
    ...
    def __init__( self, source_version ):
        ...
        self.map_dir = 'etl/extract/idc/dicom_code_maps'
        ...
        self.aux_files = {
            ...
            'collection_id_to_entity_id' : path.join( self.aux_dir, 'IDC_entity_submitter_id_to_collection_id.tsv' ),
            'file_researchsubject' : path.join( self.aux_dir, 'file_researchsubject.tsv.gz' ),
            'idc_case_id_to_submitter_case_id' : path.join( self.aux_dir, 'idc_case_id_to_submitter_case_id.tsv' ),
            'subject_idc_webapp_collection_id' : path.join( self.aux_dir, 'subject_idc_webapp_collection_id.tsv' ),
            'tcga_biospecimen_sample_barcode_to_gdc_id' : path.join( self.aux_dir, 'tcga_biospecimen_rel9.submitter_sample_id_to_gdc_id.tsv' ),
            'tcga_biospecimen_submitter_case_id_to_gdc_id' : path.join( self.aux_dir, 'tcga_biospecimen_rel9.submitter_case_id_to_gdc_id.tsv' )
        }

        self.log_files = {
            
            'no_subject_specimens' : path.join( self.log_dir, 'skipped_specimens_with_missing_subjects.tsv' )
        }
        ...
        for target_dir in [ self.output_dir, self.aux_dir, self.log_dir ]:
    ...
    def extract_file_subject_and_rs_data_from_dicom_all( self ):
        ...
        transform_log = path.join( self.log_dir, 'inconsistent_records_substitution_counts.tsv' )
        transform_block_log = path.join( self.log_dir, 'inconsistent_records_blocked_substitutions.tsv' )
