...
output_file = path.join( agg_project_dir, 'CTDC_CDA_projects_linked_to_GC_CDA_projects.tsv' )
...
ctdc_project_alias_to_gc_project_alias = dict()
...
for ctdc_project_id in sorted( ctdc_project ):
    ...
    if ctdc_project[ctdc_project_id]['type'] == 'dbgap_study' and ctdc_project_id in gc_project:
        gc_project_alias = gc_project[ctdc_project_id]['id_alias']
        if ctdc_project_alias in ctdc_project_alias_to_gc_project_alias and ctdc_project_alias_to_gc_project_alias[ctdc_project_alias] != gc_project_alias:
            # We should never get here.
            sys.exit( f"FATAL: CTDC project {ctdc_project_id} associated with two distinct GC projects: {gc_project_alias_to_project_id[ctdc_project_alias_to_gc_project_alias[ctdc_project_alias]]} ({ctdc_project_alias_to_gc_project_alias[ctdc_project_alias]}) and {gc_project_alias_to_project_id[gc_project_alias]} ({gc_project_alias}); aborting." )
        else:
            ctdc_project_alias_to_gc_project_alias[ctdc_project_alias] = gc_project_alias

# Write results.
with open( output_file, 'w' ) as OUT:
    print( *[ 'CTDC_project_alias', 'CTDC_project_id', 'GC_project_alias', 'GC_project_id' ], sep='\t', file=OUT )
    for ctdc_project_alias in sorted( ctdc_project_alias_to_gc_project_alias ):
        ctdc_project_id = ctdc_project_alias_to_project_id[ctdc_project_alias]
        gc_project_alias = ctdc_project_alias_to_gc_project_alias[ctdc_project_alias]
