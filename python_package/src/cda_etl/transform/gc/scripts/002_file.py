...
for file_uuid in file_describes_sample:
    ...
    for sample_uuid in file_describes_sample[file_uuid]:
        ...
        if sample[sample_uuid]['sample_anatomic_site'] != '':
            ...
            site_array = sorted( json.loads( sample[sample_uuid]['sample_anatomic_site'] ) )

            for site in site_array:
                if site != '':
                    file_anatomic_site[file_id].add( site )
...
for image_uuid in image_of_file:
    ...
    # GC removed this field from their `image` entity between October and December 2024.
    # 
