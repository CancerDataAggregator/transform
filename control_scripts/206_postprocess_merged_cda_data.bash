...
input_root=./cda_tsvs/merged_gdc_pdc_and_cds_002_decorated_harmonized

summary_output_dir=./auxiliary_metadata/__column_value_statistics/merged_CDA_data/GDC_${gdc_extraction_date_string}_plus_PDC_${pdc_extraction_date_string}_plus_CDS_${cds_extraction_date_string}/dataset_summaries

summary_file="${summary_output_dir}/GDC_plus_PDC_plus_CDS.converted_to_CDA.harmonized.merged.counts_by_upstream_data_source.tsv"

current_date=$(date '+%Y-%m-%d')

echo ./package_root/auxiliary_scripts/901_summarize_distinct_values_by_table_and_column.source_aware.py $input_root $summary_file "${cda_version_string}" $current_date print_CDA_rows gdc $gdc_extraction_date_string "${gdc_version_string}" pdc $pdc_extraction_date_string "${pdc_version_string}" cds $cds_extraction_date_string "${cds_version_string}"
./package_root/auxiliary_scripts/901_summarize_distinct_values_by_table_and_column.source_aware.py $input_root $summary_file "${cda_version_string}" $current_date print_CDA_rows gdc $gdc_extraction_date_string "${gdc_version_string}" pdc $pdc_extraction_date_string "${pdc_version_string}" cds $cds_extraction_date_string "${cds_version_string}"
