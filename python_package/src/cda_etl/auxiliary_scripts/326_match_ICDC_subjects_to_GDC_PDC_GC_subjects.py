...
icdc_gc_merge_map = path.join( aux_dir, 'ICDC_CDA_subjects_linked_to_GC_CDA_subjects.tsv' )
...
output_file = path.join( aux_dir, 'ICDC_CDA_subjects_merged_into_GDC_PDC_GC_CDA_subjects.tsv' )
...
merged_id = map_columns_one_to_one( gdc_pdc_gc_merge_map, 'new_subject_alias', 'new_subject_id' )
...
icdc_to_gc = map_columns_one_to_one( icdc_gc_merge_map, 'ICDC_subject_alias', 'GC_subject_alias' )
...
with open( output_file, 'w' ) as OUT:
    ...
    print( *[ 'ICDC_subject_alias', 'ICDC_subject_id', 'GDC_PDC_GC_subject_alias', 'GDC_PDC_GC_subject_id', 'new_subject_alias', 'new_subject_id' ], sep='\t', file=OUT )

    for icdc_subject_alias in sorted( icdc_subject_id ):
        
        if icdc_subject_alias in icdc_to_gc:
            
            gc_target_to_match = icdc_to_gc[icdc_subject_alias]
            ...
            if icdc_subject_alias in icdc_to_pdc:
                ...
                if pdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: ICDC subject alias {icdc_subject_alias} matched (via PDC) to {pdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
            ...
            if icdc_subject_alias in icdc_to_gdc:
                ...
                if gdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: ICDC subject alias {icdc_subject_alias} matched (via GDC) to {gdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
