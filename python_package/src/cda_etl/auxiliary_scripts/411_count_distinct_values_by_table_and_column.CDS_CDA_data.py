...
input_root = 'cda_tsvs/cds'

aux_root = 'auxiliary_metadata'

stats_dir = path.join( aux_root, '__column_stats' )

stats_file = path.join( stats_dir, 'CDS_column_stats_by_table.converted_to_CDA.tsv' )
...
with open( stats_file, 'w' ) as STATS_FILE:
    ...
    print( 'Tabulating stats for all CDA fields from CDS...', end='\n\n', file=sys.stderr )
    ...
    for file_basename in sorted( listdir( input_root ) ):
        ...
        if re.search( r'\.tsv$', file_basename ) is not None:
            ...
            distinct_value_count[file_basename] = dict()

            print( f"   ...{file_basename}...", file=sys.stderr )
            ...
            with open( input_file_path ) as IN:
                ...
                for column_name in column_names:
                    ...
                    print( *[ f"{file_basename}", column_name, str( len( column_distinct_values[column_name] ) ), str( row_count ), has_nulls, column_null_count[column_name], is_all_numbers ], sep='\t', end='\n', file=STATS_FILE )

    print( '\n...done computing general stats on CDA field values from CDS.', end='\n\n', file=sys.stderr )
