...
idc_gc_merge_map = path.join( aux_dir, 'IDC_CDA_subjects_linked_to_GC_CDA_subjects.tsv' )
...
output_file = path.join( aux_dir, 'IDC_CDA_subjects_merged_into_GDC_PDC_GC_ICDC_CDA_subjects.tsv' )
...
merged_id = map_columns_one_to_one( gdc_pdc_gc_icdc_merge_map, 'new_subject_alias', 'new_subject_id' )
...
idc_to_gc = map_columns_one_to_one( idc_gc_merge_map, 'IDC_subject_alias', 'GC_subject_alias' )
...
with open( output_file, 'w' ) as OUT:
    print( *[ 'IDC_subject_alias', 'IDC_subject_id', 'GDC_PDC_GC_ICDC_subject_alias', 'GDC_PDC_GC_ICDC_subject_id', 'new_subject_alias', 'new_subject_id' ], sep='\t', file=OUT )
    ...
    for idc_subject_alias in sorted( idc_subject_id ):
        ...
        if idc_subject_alias in idc_to_icdc:
            ...
            if idc_subject_alias in idc_to_gc:
                gc_target_to_match = idc_to_gc[idc_subject_alias]
                ...
                if gc_target_to_match != icdc_target_to_match:
                    ...
                    sys.exit( f"FATAL: IDC subject alias {idc_subject_alias} matched (via GC) to {gc_target_to_match} but also (via ICDC) to {icdc_target_to_match}; cannot continue, aborting." )
        ...
        elif idc_subject_alias in idc_to_gc:
            gc_target_to_match = idc_to_gc[idc_subject_alias]
            matching_alias = gc_target_to_match
            matching_id = gc_subject_id[matching_alias]
            # If we keep the old alias value, we don't have to rewrite existing table data. If a
            ...
            if idc_subject_alias in idc_to_pdc:
                ...
                if pdc_target_to_match != gc_target_to_match:
                    # We hope never to get here.
                    sys.exit( f"FATAL: IDC subject alias {idc_subject_alias} matched (via PDC) to {pdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
            ...
            if idc_subject_alias in idc_to_gdc:
                ...
                if gdc_target_to_match != gc_target_to_match:
                    # We hope never to get here.
                    sys.exit( f"FATAL: IDC subject alias {idc_subject_alias} matched (via GDC) to {gdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
