...
icdc_gc_merge_map = path.join( aux_dir, 'ICDC_CDA_projects_linked_to_GC_CDA_projects.tsv' )
...
output_file = path.join( aux_dir, 'ICDC_CDA_projects_merged_into_GDC_PDC_GC_CDA_projects.tsv' )
...
merged_id = map_columns_one_to_one( gdc_pdc_gc_merge_map, 'new_project_alias', 'new_project_id' )
...
icdc_to_gc = map_columns_one_to_one( icdc_gc_merge_map, 'ICDC_project_alias', 'GC_project_alias' )
...
with open( output_file, 'w' ) as OUT:
    ...
    print( *[ 'ICDC_project_alias', 'ICDC_project_id', 'GDC_PDC_GC_project_alias', 'GDC_PDC_GC_project_id', 'new_project_alias', 'new_project_id' ], sep='\t', file=OUT )

    for icdc_project_alias in sorted( icdc_project_id ):
        
        if icdc_project_alias in icdc_to_gc:
            
            gc_target_to_match = icdc_to_gc[icdc_project_alias]
            ...
            if icdc_project_alias in icdc_to_pdc:
                ...
                if pdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: ICDC project alias {icdc_project_alias} matched (via PDC) to {pdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
            ...
            if icdc_project_alias in icdc_to_gdc:
                ...
                if gdc_target_to_match != gc_target_to_match:
                    ...
                    sys.exit( f"FATAL: ICDC project alias {icdc_project_alias} matched (via GDC) to {gdc_target_to_match} but also (via GC) to {gc_target_to_match}; cannot continue, aborting." )
